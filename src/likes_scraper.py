...
class LikesScraper():
    ...
    def scrape(self, driver, args, **kwargs):
        ...
        end_cursor=parsed_json['data']['shortcode_media']['edge_liked_by']['page_info']['end_cursor']
        has_next_page=parsed_json['data']['shortcode_media']['edge_liked_by']['page_info']['has_next_page']
        ...
        while go_on and n<n_iterations:        
            if has_next_page and n<n_iterations:
                try:
                    print(n)
                    q_html=f'"shortcode"%3A"{short_code}"%2C"include_reel"%3A"true"%2C"first"%3A"{int(random.choice(f))}"%2C"after"%3A"{urllib.parse.quote_plus(str(end_cursor))}"'
                    u=f'https://www.instagram.com/graphql/query/?query_hash={query_hash}&variables={{{q_html}}}'
