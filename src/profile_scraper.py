...
class ProfileScraper():
    ...
    def parse_and_save_full_profile_raw_har(self, har, args):  
        ...
        for n in range(0, len(har['log']['entries'])):
            if 'text' in har['log']['entries'][n]['response']['content'].keys(): 
                if 'edge_owner_to_timeline_media' in har['log']['entries'][n]['response']['content']['text']:
                    raw_data.append(_finditem_nested_dict(json.loads(har['log']['entries'][n]['response']['content']['text']), 'edge_owner_to_timeline_media'))
        data=[]         
    ...
    def scrape(self, driver, proxy, args, save_raw_data=bool):
        proxy.new_har(args.target_profile, options={'captureHeaders': True, 'captureContent':True})
